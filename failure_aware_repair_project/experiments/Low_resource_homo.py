...
_LOW_RESOURCE_MODULE = None


def _load_low_resource_module():
    global _LOW_RESOURCE_MODULE
    if _LOW_RESOURCE_MODULE is not None:
        return _LOW_RESOURCE_MODULE
    module_path = Path(__file__).resolve().with_name("Low_resource.py")
    spec = importlib.util.spec_from_file_location("_failure_repair_low_resource", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load Low_resource.py from {module_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    _LOW_RESOURCE_MODULE = module
    return module
...
def _generate_low_resource_working_experiment(
    config: RandomRepairExperimentConfig,
    cache_dir: Path | None,
    ...
    if cache_dir is not None:
        low_resource = _load_low_resource_module()
        return low_resource._generate_random_repair_experiment_cached(
            source_config,
            cache_dir,
        )
...
PUBLISHED_SUMMARY_PATH = Path(__file__).resolve().with_name(
    "low_resource_homo_final_summary_v2cache.json"
)
LOW_RESOURCE_SUMMARY_PATH = Path(__file__).resolve().with_name(
...
WORKING_MAPPING_SOURCE_CACHE_DIR = Path("/private/tmp/low_resource_story_cache")

BASE_EXPERIMENT_CACHE_SCHEMA = "low_resource_homo_base_experiment_v2_workload_only"
PREVIEW_REPAIR_CACHE_SCHEMA = "low_resource_homo_preview_repair_v1"
SIMULATOR_VALIDATION_CACHE_SCHEMA = "low_resource_homo_sim_validation_v1"
SEED_STORY_CACHE_SCHEMA = "low_resource_homo_seed_story_v1"
...
SEED_STORY_CACHE_SOURCES = (
    Path("failure_aware_repair_project/experiments/Low_resource_homo.py"),
) + BASE_EXPERIMENT_CACHE_SOURCES + (
...
def _published_story_seed_priority(
    *,
    ...
    if LOW_RESOURCE_SUMMARY_PATH.exists():
        try:
            summary = json.loads(LOW_RESOURCE_SUMMARY_PATH.read_text())
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Low-resource homogeneous-GPT failure-aware repair experiment. Fixed topology is "
    ...
    parser.add_argument(
        "--story-cache-dir",
        type=Path,
        default=Path("/private/tmp/low_resource_homo_story_cache"),
...
def _preview_validation_candidate_match(
    ...
    *,
    ...
    return bool(near_advantage_story)
...
def _story_checks_from_metrics(
    ...
    *,
    ...
    story_checks["coordinate_story_success"] = bool(
        ...
        and story_checks["coordinate_advantage_meets_threshold"]
    )
    return story_checks
...
def _summarize_trials(trials: list[dict[str, object]]) -> dict[str, object]:
    ...
    for tenant_count in sorted(by_tenant):
        ...
        tenant_summary["aggregate_story_checks"]["aggregate_story_success"] = bool(
            ...
            and tenant_summary["aggregate_story_checks"][
...
def _generate_random_repair_experiment_cached(
    ...
    if cache_dir is None:
        source_experiment, _source_cache_hit = _generate_low_resource_working_experiment(
            config,
    ...
    for num_tenants in range(int(args.tenant_min), int(args.tenant_max) + 1):
        ...
        def selected_cases_meet_aggregate_story() -> bool:
            ...
            return bool(
                ...
                and statistics.mean(coordinate_advantages)
            ...
            experiment = replace(
                ...
                failure_selection_metadata={
                    ...
                    "high_impact_pool_screen": (
                        ...
                        else "estimator_coordinate_advantage"
                    ),
