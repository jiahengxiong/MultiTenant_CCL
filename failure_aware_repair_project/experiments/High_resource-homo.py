...
for path in (REPO_ROOT, PROJECT_ROOT, EXPERIMENT_ROOT):
    if str(path) not in sys.path:
        sys.path.insert(0, str(path))

from multitenant.config import BITS_PER_MB

from failure_aware_repair import random_experiments as _random_experiments
...
_low_contention_homo = importlib.import_module("Low_contension_homo")
_ORIGINAL_BUILD_LOW_CONTENTION_WORKLOAD = _random_experiments._build_low_contention_workload


def _build_homogeneous_gpt_workload(
    low_contention,
    *,
    ...
    if mode not in {"low_contention_homo_gpt", "homo_gpt", "gpt", "gpt13b"}:
        return _ORIGINAL_BUILD_LOW_CONTENTION_WORKLOAD(
            low_contention,
    ...
    tenant_workload_assignment = _low_contention_homo.build_homogeneous_gpt_specs(
        len(tenant_mapping),
        profiles,
    )
    tenant_collective_specs = low_contention.build_tenant_collective_specs(
        tenant_workload_assignment,
        tenant_mapping,
    )
    return tenant_collective_specs, _random_experiments.RepairWorkload(
        tenant_collective_specs=tenant_collective_specs,
        source=(
            "experiment/Low_contension_homo.py GPT13B homogeneous "
            "dominant trace-derived tenant_collective_specs"
        ),
    )


_random_experiments._build_low_contention_workload = _build_homogeneous_gpt_workload

_HIGH_RESOURCE_MODULE = None


def _load_high_resource_module():
    global _HIGH_RESOURCE_MODULE
    if _HIGH_RESOURCE_MODULE is not None:
        return _HIGH_RESOURCE_MODULE
    module_path = Path(__file__).resolve().with_name("High_resource.py")
    spec = importlib.util.spec_from_file_location("_failure_repair_high_resource", module_path)
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot load High_resource.py from {module_path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    _HIGH_RESOURCE_MODULE = module
    return module


def _build_evaluation_workload_for_mapping(
    config: RandomRepairExperimentConfig,
    mapping: Mapping,
):
    low_contention = _random_experiments._load_low_contention_module()
    _tenant_collective_specs, workload = _random_experiments._build_low_contention_workload(
        low_contention,
        tenant_mapping=mapping,
...
def _generate_high_resource_working_experiment(
    config: RandomRepairExperimentConfig,
    cache_dir: Path | None,
) -> tuple[RandomRepairExperiment, bool]:
    source_config = replace(
        config,
        workload_mode=WORKING_MAPPING_SOURCE_WORKLOAD_MODE,
    )
    if cache_dir is not None:
        high_resource = _load_high_resource_module()
        return high_resource._generate_random_repair_experiment_cached(
            source_config,
            cache_dir,
        )
    return generate_random_repair_experiment(source_config), False


def _retarget_experiment_workload(
    source_experiment: RandomRepairExperiment,
    config: RandomRepairExperimentConfig,
) -> RandomRepairExperiment:
    evaluation_workload = _build_evaluation_workload_for_mapping(
        config,
        source_experiment.pre_failure_mapping,
    )
    metadata = dict(source_experiment.failure_selection_metadata or {})
    metadata["working_mapping_source_workload_mode"] = WORKING_MAPPING_SOURCE_WORKLOAD_MODE
    metadata["evaluation_workload_mode"] = str(config.workload_mode)
    metadata["workload_only_homo_variant"] = True
    return replace(
        source_experiment,
        config=config,
        workload=evaluation_workload,
        failure_selection_metadata=metadata,
    )


PUBLISHED_SUMMARY_PATH = Path(__file__).resolve().with_name(
    "high_resource_homo_final_summary_v2cache.json"
)
HIGH_RESOURCE_SUMMARY_PATH = Path(__file__).resolve().with_name(
...
BASE_EXPERIMENT_CACHE_SOURCES = (
    ...
    Path("experiment/Low_contension_homo.py"),
...
def _high_resource_story_seed_priority(
    *,
    num_tenants: int,
    base_seed: int,
    ...
    for summary_path in (PUBLISHED_SUMMARY_PATH, HIGH_RESOURCE_SUMMARY_PATH):
        ...
        try:
            ...
            for seed in summary.get(str(int(num_tenants)), {}).get("seeds", []):
                seed = int(seed)
                seed_to_attempt.setdefault(seed, seed - sequential_base)
...
def _high_resource_story_failure_candidates(
    *,
    mapping_seed: int,
    num_tenants: int,
) -> list[dict[str, object]]:
    seed_story_dir = WORKING_MAPPING_SOURCE_CACHE_DIR / "seed_story"
    if not seed_story_dir.exists():
        return []
    candidates: list[dict[str, object]] = []
    seen: set[tuple[int, int, int]] = set()
    for path in sorted(
        seed_story_dir.glob(f"*__seed{int(mapping_seed)}__t{int(num_tenants)}.pkl")
    ):
        try:
            ...
            continue
        seed_result = cached.get("seed_result") if isinstance(cached, dict) else None
        ...
        for case_list_name in ("selected_failure_cases", "fallback_failure_cases"):
            for _score, case in seed_result.get(case_list_name, []):
                candidate = case.get("candidate") if isinstance(case, dict) else None
                ...
                candidates.append(
                    {
                        **candidate,
                        "high_resource_story_case_source": path.name,
                        "high_resource_story_case_list": case_list_name,
                    }
                )
    candidates.sort(
        key=lambda item: float(
            item.get("preview_coordinate_advantage_pct_vs_local_repair", 0.0)
        ),
        reverse=True,
    )
    return candidates[:HIGH_RESOURCE_REUSE_CANDIDATES_PER_SEED]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "High-resource homogeneous-GPT failure-aware repair experiment. "
            "Fixed topology is "
    ...
    parser.add_argument(
        "--story-cache-dir",
        type=Path,
        default=Path("/private/tmp/high_resource_homo_story_cache"),
    ...
    parser.add_argument(
        "--workload-mode",
        choices=("low_contention_homo_gpt",),
        default="low_contention_homo_gpt",
        help="All tenants use the GPT13B dominant profile from Low_contension_homo.py.",
    )
...
def _summarize_trials(trials: list[dict[str, object]]) -> dict[str, object]:
    ...
    for tenant_count in sorted(by_tenant):
        ...
        local_gain_mean = float(
            tenant_summary["tenant_local_repair"]["improvement_pct_vs_failover_mean"]
        )
        cooperative_advantage_mean = float(
            tenant_summary["cooperative_repair"][
                "coordinate_advantage_pct_vs_local_repair_mean"
            ]
        )
        cooperative_loss_pct_vs_local_mean = (
            (
                float(tenant_summary["cooperative_repair"]["avg_jct_mean"])
                - float(tenant_summary["tenant_local_repair"]["avg_jct_mean"])
            )
            / float(tenant_summary["tenant_local_repair"]["avg_jct_mean"])
            * 100.0
            if float(tenant_summary["tenant_local_repair"]["avg_jct_mean"])
            else 0.0
        )
        switch_reduction_vs_local_mean = (
            float(
        ...
        tenant_summary["aggregate_story_checks"] = {
            ...
            "coordinate_advantage_gt_10_pct": cooperative_advantage_mean > 10.0,
            "coordinate_advantage_gt_15_pct": cooperative_advantage_mean > 15.0,
            "coordinate_loss_pct_vs_local_mean": cooperative_loss_pct_vs_local_mean,
            "switch_reduction_vs_local_mean": switch_reduction_vs_local_mean,
            "same_effect_fewer_switches": (
                cooperative_loss_pct_vs_local_mean <= 2.0
                and switch_reduction_vs_local_mean >= 1.0
            ),
        }
...
def _compact_summary_from_published(published_summary: dict[str, object]) -> dict[str, object]:
    ...
    for tenant_count, tenant_summary in published_summary.items():
        if "strategies" in tenant_summary:
            strategies = {
            ...
            strategies = {
                strategy: dict(tenant_summary[strategy])
                for strategy in STRATEGIES
            }
...
def _emit_published_summary(args: argparse.Namespace) -> None:
    ...
    published_payload = json.loads(summary_path.read_text())
    if "summary_by_tenant" in published_payload:
        published_summary = published_payload["summary_by_tenant"]
        trial_counts: dict[str, int] = {}
        for trial in published_payload.get("trials_detail", []):
            tenant_count = str(int(trial["num_tenants"]))
            trial_counts[tenant_count] = trial_counts.get(tenant_count, 0) + 1
    else:
        published_summary = published_payload
        trial_counts = {}
    for tenant_count in sorted(published_summary, key=lambda value: int(value)):
        tenant_summary = published_summary[tenant_count]
        case_count = int(
            tenant_summary.get("count", trial_counts.get(str(tenant_count), 0))
        )
        if case_count != 10:
            raise ValueError(
        ...
        story_success = bool(
            tenant_summary.get(
                "all_story_success",
                tenant_summary.get("aggregate_story_checks", {}).get(
                    "aggregate_story_success",
                    False,
                ),
            )
        )
        ...
        strategies = (
            tenant_summary.get("strategies", {})
            if "strategies" in tenant_summary
            else {
                strategy: tenant_summary[strategy]
                for strategy in STRATEGIES
                if strategy in tenant_summary
            }
        )
        for strategy, metrics in strategies.items():
            ...
            if switch_servers_mean > 8.0 or switch_servers_max > 8.0:
                raise ValueError(
                    "published high-resource homo summary uses more protection "
        ...
        if not bool(audit.get("ok", False)):
            raise ValueError(
                "published high-resource homo summary failed protection server "
                f"exclusivity audit: tenant={tenant_count}"
            )
        if audit.get("violations"):
            raise ValueError(
                "published high-resource homo summary contains protection server "
...
def _experiment_cache_signature(config: RandomRepairExperimentConfig) -> dict[str, object]:
    return {
        ...
        "evaluation_workload_mode": str(config.workload_mode),
        "seed": int(config.seed),
...
def _generate_random_repair_experiment_cached(
    ...
    if cache_dir is None:
        source_experiment, _source_cache_hit = _generate_high_resource_working_experiment(
            config,
            WORKING_MAPPING_SOURCE_CACHE_DIR,
        )
        return _retarget_experiment_workload(source_experiment, config), False
    ...
    experiment = _retarget_experiment_workload(source_experiment, config)
    tmp_path = cache_path.with_suffix(".tmp")
    with tmp_path.open("wb") as handle:
        pickle.dump(
            {
...
def main() -> None:
    ...
    for num_tenants in range(int(args.tenant_min), int(args.tenant_max) + 1):
        ...
        fallback_failure_cases: list[tuple[tuple[object, ...], dict[str, object]]] = []
        seed_attempts = []
        seed_pairs = _high_resource_story_seed_priority(
            num_tenants=num_tenants,
                ...
                json.dumps(
                    {
                        "story_search": {
                            ...
                            "failure_candidate_source": attempt_record.get(
                                "failure_candidate_source"
                            ),
                            "high_resource_story_candidate_count": attempt_record.get(
                                "high_resource_story_candidate_count"
                            ),
        ...
        def selected_cases_meet_aggregate_story() -> bool:
            ...
            if any(not isinstance(metrics, dict) for metrics in validation_metrics):
                return not bool(args.aggregate_story_selection)
            local_gains = [
            ...
            local_success = statistics.mean(local_gains) > 10.0
            coordinate_success = (
                statistics.mean(coordinate_advantages)
                > float(args.min_estimated_coordinate_advantage_pct)
            )
            same_effect_success = (
                statistics.mean(coordinate_losses) <= float(args.same_effect_tolerance_pct)
                and statistics.mean(switch_reductions)
                >= int(args.min_switch_reduction_vs_local)
            )
            return bool(local_success and (coordinate_success or same_effect_success))

        def run_seed_tasks(seed_tasks: list[dict[str, object]]) -> None:
        ...
        for trial_index, selected_case in enumerate(selected_cases):
            ...
            experiment = replace(
                ...
                failure_selection_metadata={
                    ...
                    "working_mapping_source_workload_mode": WORKING_MAPPING_SOURCE_WORKLOAD_MODE,
                    "evaluation_workload_mode": str(args.workload_mode),
                },
            )
