...
def _run_single_case_worker(payload: dict[str, object]) -> dict[str, object]:
    print(
        f"[worker pid={os.getpid()}] high_contention_homogeneous "
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="High-contention homogeneous comparison: all tenants use GPT13B dominant DP collective.",
    )
...
def main() -> None:
    ...
    profiles = load_dominant_profiles(4)
    cpu_count = os.cpu_count() or 1
    pool_size = min(7, cpu_count)

    all_work_items: list[dict[str, object]] = []
    results = []
    for tenant_count in tenant_counts:
        print(f"=== high_contention_homogeneous tenant_count={tenant_count} ===", flush=True)
        tenant_workload_assignment = build_homogeneous_gpt_specs(tenant_count, profiles)
        for trial_index in range(args.trials):
            trial_seed = derive_seed(args.seed, "high_contention_homogeneous", "trial", tenant_count, trial_index)
            print(
                f"--- high_contention_homogeneous tenant_count={tenant_count} trial={trial_index + 1}/{args.trials} seed={trial_seed} ---",
                flush=True,
            )
    ...
    with Pool(processes=pool_size, maxtasksperchild=1) as pool:
        ...
        for completed_idx, case in enumerate(pool.imap_unordered(_run_single_case_worker, all_work_items), start=1):
            print(
                f"[progress] high_contention_homogeneous tenant_count={int(case['tenant_count'])} "
    ...
    for tenant_count in tenant_counts:
        ...
        results.append(
            {
                "tenant_count": tenant_count,
                "scene": "high_contention_homogeneous",
    ...
    payload = {
        "metadata": {
            "scene": "high_contention_homogeneous",
            "definition": "each server is occupied by exactly two distinct tenants; all tenants use GPT13B dominant DP collective",
            "topology": TOPOLOGY,
            ...
            "task_size_rule": "GPT13B dominant DP collective only; msg_size is in bytes; task_size_bits = (msg_size_bytes * 4) / occupied_servers",
