...
class MappingILPSolver:
    ...
    def _apply_serial_schedule_warm_start(self, mapping):
        ...
        while any(remaining.values()):
            ...
            bottleneck_links = tuple(edge for edge in edges if abs(float(capacities[edge]) - bottleneck) <= 1e-12)
            selected_bottleneck_link = bottleneck_links[0] if bottleneck_links else None
            ...
            task_bottleneck_links[(tenant, task_id, "selected")] = selected_bottleneck_link
    ...
    def _maybe_seed_from_heuristic(self, time_limit):
        ...
        if self.warm_start_mapping is not None:
            return
        if not has_collective_workload(
...
class MappingHeuristicSolver:
    ...
    def _path_edges_for_pair(self, path_edges, src_server, dst_server):
        key = (int(src_server), int(dst_server))
        edges = path_edges.get(key)
        if edges is None:
            edges = tuple(
                tuple(int(node) for node in edge)
                for edge in self.datacenter.ECMP_table[key]
            )
